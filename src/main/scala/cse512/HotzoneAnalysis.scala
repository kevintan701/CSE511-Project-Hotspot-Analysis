...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    // Count points per rectangle and sort by rectangle string in ascending order
    val resultDf = spark.sql("select rectangle, count(*) as count from joinResult group by rectangle order by rectangle asc")
